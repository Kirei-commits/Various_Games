...
- 検証: lint OK / ロジック 39件 / E2E 44件（desktop + mobile）/ 通しプレイ 0 issues
- 育ちの速さ（`npm run simulate` 10分×10回の実測）
  - レベル中央 **14**（3分では中央8）、稼いだコイン中央 24,746、流れた注文 0
  - 何もできない時間 **0ms**、救済の発動 **0回**
- レベル20（上限）までは、手を止めずに回して約17分
...
3. **gungun-farm: 人が遊んだときのペース。** 通しプレイ（実ブラウザ・3分）ではレベル4止まり。
   シミュレーションの上手な回し方とは倍くらい差がある。畑の一括操作をもう一段楽にするか、
   序盤の経験値を厚くするかを、**測ってから**決める
