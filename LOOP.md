...
- 検証: lint OK / ロジック 56件 / E2E 68件（desktop + mobile）/ 通しプレイ 0 issues
...
3. **gungun-farm: 注文が脇役。** 届けるより売ったほうが早い場面が多い。
   「届けることが主筋」になる形を探す。詳しくは `gungun-farm/ROADMAP.md` の A-1
4. **gungun-farm: 畑の手数がまだ重い。** 通しプレイで収穫281回に対し加工11回・配達12回
   （「ぜんぶ仕込む」を入れる前は加工3回・配達3回だったので改善はしている）
