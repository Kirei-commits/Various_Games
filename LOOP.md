...
- 検証: lint OK / ロジック 79件 / E2E 76件（desktop + mobile）/ 通しプレイ 0 issues ×2本
- **中身**: 作物11種・施設18台・品物29種。**Lv1〜20 のどのレベルにも解放がある**
  （Lv14〜20 が7レベル空っぽだったのを埋めた。lint が再発を弾く）
- **待ち時間（このゲームの売り・`npm run simulate` が毎回測る）**
  - 1秒以上つづいた待ち **0.1%**（時間差まきの前は 8.1%）
  - いちばん長く待たされた時間 **1300ms**（前は 3500ms）。3秒を超えたら失敗する
- 育ちの速さ: 10分でレベル中央 **14**、レベル20まで約18分（25分で施設18台中17台）
- 面白さの指標: 判断のある操作の割合 **24.9%**（収穫と植え直しを畳む前は 11.5%）
- **タネ選びが取引になっている**: 短い作物は稼ぎ1.28倍・手数3.75倍、
  長い作物は経験値1.13倍で手数4分の1。どちらも全部では勝たない（毎回テストで確認）
- **注文が主筋になった**（ふなびん導入）: コインの出どころが
  注文 44.6% → **65.8%**、余りを売る 55.4% → 34.2%
- **連打は損**（実測）: 200ms間隔より800ms間隔のほうが1.34倍稼げる。
  400〜1500msのどこでも110〜134%で、狙って合わせる必要が無い
- 一日が3分で一巡（朝→昼→夕→夜）／実績15個／収穫の音が段を上がる
...
3. **gungun-farm: 畑の手数がまだ重い。** 通しプレイで収穫267回に対し加工3回・配達2回
   （Lv12から始めると加工22回・配達3回・船に80個）。詳しくは `gungun-farm/ROADMAP.md` の A-1
...
| `ci.yml` / 通しプレイ | 〃 | `playtest:ci` を持つゲームだけ。実ブラウザで最後まで遊ぶ（gungun-farm は最初からの1本と、`--level 12` で後半を踏む1本） |
