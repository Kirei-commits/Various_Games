...
(function (global) {
  ...
  const ITEMS = {
    ...
    soy:     { name: 'だいず',       emoji: '🫘', sell: 28 },
    cane:    { name: 'さとうきび',   emoji: '🎋', sell: 36 },
    berry:   { name: 'いちご',       emoji: '🍓', sell: 44 },
    pumpkin: { name: 'かぼちゃ',     emoji: '🎃', sell: 54 },
    grape:   { name: 'ぶどう',       emoji: '🍇', sell: 66 },
    ...
    jam:     { name: 'ジャム',       emoji: '🍯', sell: 195 },
    cake:    { name: 'ケーキ',       emoji: '🍰', sell: 321 },
    pie:     { name: 'パイ',         emoji: '🥧', sell: 154 },

    // レベル14以降。ここから先が空っぽだと「どんどん増える」が途中で止まる
    tomato:   { name: 'トマト',       emoji: '🍅', sell: 90 },
    olive:    { name: 'オリーブ',     emoji: '🫒', sell: 114 },
    melon:    { name: 'メロン',       emoji: '🍈', sell: 142 },
    ketchup:  { name: 'ケチャップ',   emoji: '🥫', sell: 265 },
    pasta:    { name: 'パスタ',       emoji: '🍝', sell: 153 },
    oil:      { name: 'オリーブ油',   emoji: '🧴', sell: 335 },
    pizza:    { name: 'ピザ',         emoji: '🍕', sell: 785 },
    icecream: { name: 'アイス',       emoji: '🍨', sell: 508 },
    salad:    { name: 'サラダ',       emoji: '🥗', sell: 640 },
   ...
   *
   * **どの作物も1秒。** 以前は2〜9秒の幅を持たせ、「短いほど1秒あたりが良い・
   * 長いほど1枠の値打ちが高い」という取引にしていたが、遊んでみると
   * **ただ待たされるだけ**だった。秒数で差をつけるのをやめる。
   *
   * 代わりに、**上位の作物ほど もうけ（売値−タネ代）が大きく、タネ代も高い**。
   * タネ代は畑の数だけ毎秒出ていくので、上位作物に切り替えるには手元の資金が要る。
   * 「いま何を植えるか」は、**注文とふなびんが欲しがっているもの**と、
   * **タネ代を払い続けられるか**で決まる。
   */
  const CROPS = [
    { id: 'wheat',     sec: 1, cost:   3, xp:  1, level:  1 },
    { id: 'carrot',    sec: 1, cost:   5, xp:  2, level:  1 },
    { id: 'corn',      sec: 1, cost:   7, xp:  3, level:  2 },
    { id: 'soy',       sec: 1, cost:  10, xp:  4, level:  4 },
    { id: 'cane',      sec: 1, cost:  14, xp:  5, level:  6 },
    { id: 'berry',     sec: 1, cost:  17, xp:  6, level:  8 },
    { id: 'pumpkin',   sec: 1, cost:  21, xp:  7, level: 10 },
    { id: 'grape',     sec: 1, cost:  26, xp:  8, level: 12 },
    { id: 'tomato',    sec: 1, cost:  42, xp:  9, level: 14 },
    { id: 'olive',     sec: 1, cost:  56, xp: 10, level: 16 },
    { id: 'melon',     sec: 1, cost:  72, xp: 11, level: 18 }
  ];
  ...
  const BARN_AT_START = 150;
  const BARN_STEP = 50;
  const barnPrice = (upgrades) => Math.round(500 * Math.pow(1.7, upgrades));
   ...
   *  120*level^1.35 … 作物をぜんぶ1秒にしたら収穫が2倍になり、10分でレベル19に着いた
   */
  const xpFor = (level) => Math.round(120 * Math.pow(level, 1.35)) + 40;
