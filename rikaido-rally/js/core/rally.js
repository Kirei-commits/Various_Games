 ...
 */  import { MIN_LEVEL, MAX_LEVEL, unitLabel } from './banks.js';
import * as Judge from './judge.js';
import * as Hint from './hint.js';
import * as Grade from './grade.js';

export const SIZE = 5;              // 1ラリーの問題数
export const START_LEVEL = 2;       // 1問目のレベル。1にすると全員が満点付近に寄って差が出ない
...
export function pick(bank, plan, usedIds, target, rng) {
...
/** ラリーを1つ始める */
export function create(bank, history, seed) {
  ...
  const session = {
    bank, bankId: bank.id, bankName: bank.name, seed,
